...
if ( ! function_exists( 'poc_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function poc_posted_on() {
...
if ( ! function_exists( 'poc_entry_footer' ) ) :
/**
...
// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && poc_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'poc' ) . '</span><span class="sep">|</span>', $categories_list ); // WPCS: XSS OK.
		}
 ...
 * @return bool
 */
function poc_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'poc_categories' ) ) ) {
		...
		set_transient( 'poc_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so poc_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so poc_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in poc_categorized_blog.
 */
function poc_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'poc_categories' );
}
add_action( 'edit_category', 'poc_category_transient_flusher' );
add_action( 'save_post',     'poc_category_transient_flusher' );
